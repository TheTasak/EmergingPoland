class TreeChartUdzial{
  ...
  #draw_chart = () => {
    ...
    this.svg.selectAll('.treechart-chunk')
        .filter(d => {return d.data.name != this.current_data.name;})
  			.on("mousemove", (ev, d) => {
  				let tooltipsize = [(d.data.translate.length + String(d.year).length)*12, this.height / 8];
          ...
          tooltip
            ...
            .style("opacity", "0.7");
  			...
  			tooltiptext
  				...
  				.text(d.data.translate + " " + d.data.year);
  ...
  #draw_table = () => {
    ...
    for(let i = 0; i < data_children.length; i++){
      data_string += "<tr><td align='center'>" + data_children[i].translate + "</td><td align='right'>" + data_children[i].year + "</td></tr>";
    }
    data_string += "</table>";
